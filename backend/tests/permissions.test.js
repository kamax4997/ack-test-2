  ...
  it('should have permissions with isAllowed on Field', (done) => {
    this
      ...
      .then((res) => {
        expect(res.status).toBe(200);
        expect(res.success).toBe(true);
        expect(res.data.users.filter(u => !!u.email && !testUsersUsernames.includes(u.username))).toHaveLength(mockUsers.length);
        done();
      })
      .catch((err) => {
        expect(err).toBe(null);
  ...
  it('should NOT have permissions with isAllowed on Field [Return null for the field and response status 200]', (done) => {
    this
      ...
      .then((res) => {
        expect(res.status).toBe(200);
        expect(res.success).toBe(true);
        expect(res.data.users.filter(u =>  !testUsersUsernames.includes(u.username))).toHaveLength(mockUsers.length);
